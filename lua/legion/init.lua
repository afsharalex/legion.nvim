--- Legion: Neovim Plugin for Claude Code Integration
-- @module legion
--
...
function M.setup(user_config)
  ...
  log.info("Legion plugin initializing (v" .. M.version .. ")")
  ...
  -- Setup highlights
  local highlights = require("legion.ui.highlights")
  highlights.setup()
  ...
  if not provider.is_available() then
    vim.notify(
      "Legion: claude_agent_sdk not found. Set config.sdk_path or add to runtimepath.",
      vim.log.levels.WARN
    )
  end
end
...
-- Note: Commands are defined in plugin/legion.lua for lazy.nvim auto-detection
-- This function is kept for compatibility but commands are already registered
function M._create_commands()
  -- Commands are defined in plugin/legion.lua so lazy.nvim can auto-detect them
  -- Nothing to do here
end
...
function M._setup_keymaps()
  ...
  if keymaps.visual then
    vim.keymap.set("v", keymaps.visual, function()
      ...
      vim.ui.input({ prompt = "Legion instruction: " }, function(input)
  ...
  if keymaps.cancel then
    vim.keymap.set("n", keymaps.cancel, function()
      M.cancel_all()
    end, { desc = "Cancel all Legion operations" })
  end
  ...
  if keymaps.line then
    vim.keymap.set("n", keymaps.line, function()
      ...
      local row = cursor[1]
      vim.ui.input({ prompt = "Legion line instruction: " }, function(input)
        if input and #input > 0 then
...
function M.cancel_all()
  ...
  if count == 0 then
    vim.notify("Legion: No active operations", vim.log.levels.INFO)
    return
  end

  cleanup.cleanup_all()
  vim.notify(string.format("Legion: Cancelled %d operation(s)", count), vim.log.levels.INFO)
end
...
M.config = require("legion.config")
M.state = require("legion.state")
